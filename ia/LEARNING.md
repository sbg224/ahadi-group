...
Date : 2026-07-25 18:59
Contexte : Tentative de durcissement de la CSP (`next.config.ts`) en retirant `'unsafe-inline'` de `script-src`, remplacé par l'intégrité par hash (Subresource Integrity) via `experimental.sri` (Next.js 16.2.11). Un test fonctionnel initial par `curl` avait seulement confirmé le code HTTP 200 et la présence de l'en-tête CSP, sans exécuter de JavaScript.
Observation : Après build et vérification en navigateur réel (Chrome headless), `experimental.sri` ajoute correctement l'attribut `integrity` aux balises `<script src="...">` externes, mais ne couvre pas les scripts inline `self.__next_f.push(...)` que Next.js injecte pour streamer le payload React Server Components. Sans `'unsafe-inline'`, un nonce ou un hash pour ces scripts précis, la CSP les bloque : 9 violations distinctes observées dans la console, une par script inline.
Enseignement : Un test `curl` valide un en-tête HTTP, pas l'exécution JavaScript ni le respect effectif d'une CSP par un navigateur ; seule une exécution réelle (navigateur, headless ou non) le permet. Par ailleurs, la fonctionnalité SRI expérimentale de Next.js 16 protège uniquement les scripts externes avec `src` ; elle ne permet pas, à elle seule, de retirer `'unsafe-inline'` de `script-src` sur ce projet, car le mécanisme de streaming RSC dépend de scripts inline non couverts. L'alternative par nonce (voir doc Next.js, CSP §Nonces) exigerait de passer toutes les pages en rendu dynamique, un compromis jugé disproportionné pour ce site vitrine à pages statiques, sans réinjection de contenu utilisateur.
Recommandation : Conserver `'unsafe-inline'` dans `script-src` pour ce projet tant que la génération statique reste souhaitée. Avant toute nouvelle tentative de durcissement de la CSP, vérifier en navigateur réel (pas seulement `curl`) l'absence de violation, en particulier sur les scripts inline générés par le framework lui-même (RSC, hydratation), pas seulement sur les scripts applicatifs.
Références : AGENTS.md (documentation locale Next.js non conventionnelle).